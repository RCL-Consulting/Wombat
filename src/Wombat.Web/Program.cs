...
await using (var scope = app.Services.CreateAsyncScope())
{
    ...
    {
        var resetFlagIndex = Array.IndexOf(args, "--dev-reset-password");
        if (resetFlagIndex >= 0)
        {
            if (resetFlagIndex + 2 >= args.Length)
            {
                throw new InvalidOperationException(
                    "--dev-reset-password requires two trailing arguments: <email> <new-password>.");
            }

            var targetEmail = args[resetFlagIndex + 1];
            var newPassword = args[resetFlagIndex + 2];

            var userManager = scope.ServiceProvider
                .GetRequiredService<UserManager<WombatIdentityUser>>();
            var targetUser = await userManager.FindByEmailAsync(targetEmail)
                ?? throw new InvalidOperationException($"No user with email '{targetEmail}'.");

            var token = await userManager.GeneratePasswordResetTokenAsync(targetUser);
            var resetResult = await userManager.ResetPasswordAsync(targetUser, token, newPassword);
            if (!resetResult.Succeeded)
            {
                var errors = string.Join("; ", resetResult.Errors.Select(error => error.Description));
                throw new InvalidOperationException(
                    $"Failed to reset password for '{targetEmail}': {errors}");
            }

            Console.WriteLine($"Password reset for '{targetEmail}'.");
            return;
        }

        var addRoleFlagIndex = Array.IndexOf(args, "--dev-add-role");
        if (addRoleFlagIndex >= 0)
        {
            if (addRoleFlagIndex + 2 >= args.Length)
            {
                throw new InvalidOperationException(
                    "--dev-add-role requires two trailing arguments: <email> <role>.");
            }

            var targetEmail = args[addRoleFlagIndex + 1];
            var roleToAdd = args[addRoleFlagIndex + 2];
            ...
            if (await userManager.IsInRoleAsync(targetUser, roleToAdd))
            {
                Console.WriteLine($"User '{targetEmail}' already has role '{roleToAdd}'.");
                return;
            }

            var addRoleResult = await userManager.AddToRoleAsync(targetUser, roleToAdd);
            if (!addRoleResult.Succeeded)
            {
                var errors = string.Join("; ", addRoleResult.Errors.Select(error => error.Description));
                throw new InvalidOperationException(
                    $"Failed to add role '{roleToAdd}' to '{targetEmail}': {errors}");
            }

            Console.WriteLine($"Role '{roleToAdd}' added to '{targetEmail}'.");
            return;
        }
    }
