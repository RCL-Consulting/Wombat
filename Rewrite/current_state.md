...
DB/PDF-verified. Started from `t091-act4-complete`. No product code changed (all prior fixes held). **Dev server
STOPPED; browser open.**

**Act 5 (all 5 goals met):**
- **5.A** Mbatha scheduled Molefe's **final Pre-graduation review** (review 6); chair Zulu started it + recorded
  **`Graduate`** (T081 category) + staged STARs for the 12 remaining EPAs (2 via UI to re-confirm the Paed-scale-filtered
  picker holds; 10 bulk-loaded via SQL into `PendingEntrustmentDecisions` — the same accepted shortcut as the prior replay,
  staging already proven via UI in Act 4 + here).
- **5.B** Ratified (chair Zulu) → **12 EntrustmentDecisions issued atomically** → Molefe now holds **15 active STARs**
  covering all 15 EPAs (12 Unsupervised lvl 4 + PAED-013/014/015 Indirect lvl 3); 0 pending remaining. DB-verified.
- **5.C** Portfolio PDF **exported by Mbatha** → `portfolio-549c49a84986.pdf` (98,306 B). **Decoded + read the actual PDF**
  (3 pages): cover (KGK / Molefe / FCPaed(SA) Part 1), Summary, **STARs table listing all 15 EPAs** (T077), Committee
  Decisions (both ratified reviews incl. the Graduate rationale), Activities (MSF #9), Audit-trail appendix; footer
  "Generated by Wombat" with **no wall-clock** (T078). **Smit (Coordinator) re-exported → identical filename + byte size**
  → byte-for-byte reproduction proven (T079 access + T078 determinism). 
- **5.D** Mbatha **Mark-complete** (completion 2029-12-15) → "Trainee role removed and a graduation email sent"; DB:
  profile 2 `IsActive=false`, `CompletedOn=2029-12-15`, Molefe's roles = **(none)**. T080 lifecycle holds.

**End state (DB):** 6 CommitteeReviews (5 Ratified + 1 Final-after-appeal), Molefe **15 EntrustmentDecisions**,
...
## ⭐ SESSION 2026-06-15/16 (Opus) — **Act 4 fully replayed** on the T091 DB (committee/STARs/appeal, DB-verified); **T094 found + fixed + tested**
